...
def getCirclesCN(locs, rgb_testim, ori_im, display=display_default):
    ...
    for l in range(len(circles)):
        cv2.circle(rgb1, (int(round(circles[l,0] + w/2)), int(round(circles[l,1] + h/2))), 1, (0,255,255), 5)
    if display:
        plt.figure()
...
def sphereSepReflct(scale_given, pixel_for_scale, numOfCircles, circles, meanDiameter, display=display_default):
    ...
    reflection = np.square((((0.793*(meanDiameter/avgSphereSepPixel)**(2))+1)**(3/2)-(1.52))/
                (((0.793*(meanDiameter/avgSphereSepPixel)**(2))+1)**(3/2)+(1.52)))
    if display:
...
def sizeOfUncoveredRegion(rgb, mean_Diameter, img_bw, display=display_default):
    ...
    _, contours, hierarchy = cv2.findContours( #note first return is image, changed because new version
        image = img_bw,
        mode = cv2.RETR_TREE,
...
if __name__ == "__main__":
    ...
    tmplim_loc ='AllImages\\templates\\'

    #Sample test and template image path
    # testImage = 'AllImages\\test\\testImages_1.png'
    # tmplImage = 'AllImages\\templates\\template_1.jpeg'
    ...
    for idx in range(0,len(alltestimages)):
        ...
        circlesHCDiameterPixel, circlesHCDiameterActual = avgCircleDiameter(scale_given=3, pixel_for_scale = 84, circles=circlesHC, display=display_default)
        print("Average circle diameter in pixel and micrometer, detected using hough circle method :: ", str(circlesHCDiameterPixel), str(round(circlesHCDiameterActual,3)))
        ...
        avgSphereSepPixel, avgSphereSepMM, reflection, cds, distances = sphereSepReflct(scale_given=3, pixel_for_scale=84, numOfCircles=len(circlesHC[0]), circles=circlesHC, meanDiameter=circlesHCDiameterPixel, display=display_default)
        ...
        contour_list = sizeOfUncoveredRegion(rgb = tt_rgb, mean_Diameter=circlesHCDiameterPixel,img_bw=imgBW, display=display_default)
        print("Number of Contours found :: " + str(len(contour_list)))
        allcounts, dlCount = numDoubleLayers(dist = distances, circles=circlesHC, rgb=tt_rgb, mean_Diameter=circlesHCDiameterPixel, circleDiameters=cds,display=display_default)
