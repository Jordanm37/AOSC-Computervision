...
def load_images(filepath):
    ...
    for fileName in fileNames:
        print(fileName)
        if "right_" in fileName:  # right_ is for the file type in the test folder may be removed if files are named properly
    ...
    for i in range(0, nn):
        ...
        for j in range(0, nn):
            ...
            if not len(SearchWin) > 1:
                continue
            y, x = get_MultipassCrossCorrelation(TempWin, SearchWin, len_list, TempWinCen[0], TempWinCen[1])
            print(y)
