...
def main():
    ...
    distFromCam = { "2000" : 2000, "1980" : 1980, "1960" : 1960, "1940" : 1940, "1920" : 1920 , "1900" : 1900 }
    d = distFromCam.items()
    # print(d)
    # for k,v in d:
    ...
    for dist_str, dist in d: 
        im_orig_l[dist] = cv2.imread('cal_images\\cal_image_left_' + dist_str + '.tiff')
        im_orig_r[dist] = cv2.imread('cal_images\\cal_image_right_' + dist_str + '.tiff')
        C_l[dist], d_l[dist] = fun1(img_orig_l[dist], img_template)
        C_r[dist], d_r[dist] = fun1(img_orig_r[dist], img_template)
        z_factor[dist] = 0.5*(d_l[dist]+d_r[dist])/dist 
    ...
    coefficients = []
    Prediction = []
    for dist_str, dist in distFromCam.items(): 
        print('\n\nfor image_' + dist_str )
        coefficients[dist], Prediction[dist] = fun_predict(C_l[dist],C_r[dist],op,z_factor[dist])
