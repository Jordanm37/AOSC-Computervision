...
def zero_padding( len_pattern, template ):
    """
...
def zero_padding_lib( pad, template ):
    """
    Pad 1D template at begining and end of array with pattern length
 
    Inputs:
    ----------------
        pad   Pattern must be non empty and sum-square of its elements precalculated and passed
...
def norm_cross_corr( pattern, template, debug = False ): #change later to signal 1 and 2 as inputs
    ...
    template_padded = zero_padding( len_p, template )
...
def main():
    ...
    #Read signal data
    S1Data = np.array(read_file( "sensor1Data.txt") )
    S2Data = np.array(read_file( "sensor2Data.txt") )
