...
def get_grid(image, numY, numX):
    '''
    Task 2.a.
    This  Function Receives one image and devide the image 
    to multiple windows based on numY and numX as number of 
    widows in y and x  axis 
...
def get_grid_overlap(image, numY, numX, overlap):
    '''
    Task 2.b.
    This  Function Receives one image and divide the 
    image to multiple windows based on numY and numX  
    and Overlap
...
def get_SearchArea(image, loc, winSize, winNo):
    '''
    Task 3.a.
    This  Function Receives one image and retuns an subsection 
    of the image cented at loc and of size depencing on 
    winsize and winNo winSize is the size of the original 
    windo. and win no is the no of repetation  
...
def get_SearchArea_Horizontal(image, loc, winSize, winNo):
    ...
    Ylen = int(winSize[0] / 2) #* 1.5 instead
    Xlen = int(winSize[1] * winNo / 2)  #* 1.5 instead
...
def get_MultipassCrossCorrelation(pattern, template, winSize, ygrid, xgrid):
    '''
    Task 4.
    This  Function Receives two image pattern and template 
    first search the pattern in template and get an distantance 
    then devide the pattern in 4 parts and also reduce the 
    template size and searches for the individual 4 sub section 
    and get more accurate distance values.
    this Function returns two (2x2) array for y and x axis 
    counting in the respective distance  
    '''
